...
final class PieChartData: ObservableObject {
    @Published private(set) var data: [SlideData] = []
    
    init(data: [Double], colors: [UIColor]) {
        ...
        for index in 0..<data.count {
            let slide = SlideData()
            let dataItem = DataItem(name: "Data name \(index + 1)", value: data[index], color: Color(colors[index]))
